...
> ⚠️ **重要声明**
>
> 本文档由 url-extract v2.5（学术测试版）生成。当视频无字幕时，文档中的部分内容系从社区中相同或相似标题的公开资料处获取并整合而成，**并非直接由视频本身的音频或画面提取**。部分要点可能来自社区作者的二次解读，而非视频创作者的原话。视频中独有的演示画面、语气强调、即兴发挥等内容无法被覆盖。文档的结构化呈现可能掩盖内容可信度的局限性。
>
...
10. **文档尾部声明（强制）**：所有生成的 Markdown 文档**必须**在尾部以 `---` 分隔后附上完整的重要声明（见各模板尾部）。声明内容不可省略、不可缩减、不可改写。即使内容来源包含完整字幕（直接来自视频），声明仍必须附上。
11. **学术测试版标注**：文档中涉及"版本"的描述均应标注为"学术测试版（Academic Preview）"，不得省略此标注。
